...
export const ProductCard: React.FC<ProductCardProps> = ({ product, variant = 'default', onClick }) => {
  ...
  return (
    <div
      ...
      className={`group flex h-full flex-col bg-white rounded-xl overflow-hidden hover:shadow-md transition-shadow border border-brand-dark/5 ${onClick ? 'cursor-pointer focus:outline-none focus:ring-2 focus:ring-brand-gold focus:ring-offset-2' : ''}`}
    >
      {/* Image Section */}
      ...
      {/* Content Section */}
      <div className={`${isCompact ? 'p-2 gap-1' : 'p-3 gap-1'} flex flex-1 flex-col border-t border-brand-dark/5 bg-white`}>
        <h4 className={`font-bold text-brand-dark line-clamp-2 ${isCompact ? 'text-xs min-h-8' : 'text-base h-12'}`} title={product.name}>
          {product.name}
        </h4>

        <div className={`${isCompact ? 'mt-auto pt-3' : 'mt-auto pt-4'}`}>
